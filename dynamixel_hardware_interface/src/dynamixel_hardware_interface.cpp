  ...
  [[maybe_unused]] const rclcpp::Time & time, const rclcpp::Duration & period)
{
  ...
  // Loguear valor de Present Current para cada joint
  for (const auto& state : hdl_trans_states_) {
    for (size_t i = 0; i < state.interface_name_vec.size(); ++i) {
      if (state.interface_name_vec[i] == "Present Current") {
        RCLCPP_INFO(logger_, "READ: joint=%s, id=%d, Present Current=%f", 
          state.name.c_str(), state.id, *state.value_ptr_vec[i]);
      }
    }
  }
  ...
  [[maybe_unused]] const rclcpp::Time & time, const rclcpp::Duration & period)
{
  if (dxl_status_ == DXL_OK || dxl_status_ == HW_ERROR) {
    ...
    // Después de CalcJointToTransmission();
    for (const auto& cmd : hdl_trans_commands_) {
      for (size_t i = 0; i < cmd.interface_name_vec.size(); i++) {
        RCLCPP_INFO(logger_, "WRITE: joint=%s, id=%d, interface=%s, value=%f", 
          cmd.name.c_str(), cmd.id, cmd.interface_name_vec[i].c_str(), *cmd.value_ptr_vec[i]);
      }
    }
  ...
  for (const hardware_interface::ComponentInfo & gpio : info_.gpios) {
    ...
    for (const auto & param : gpio.parameters) {
      ...
      if (param_name == "Operating Mode") {
        int op_mode = stoi(param.second);
        // ¡Aquí imprime el modo de operación ANTES de setearlo!
        RCLCPP_INFO_STREAM(logger_, "[ID:" << static_cast<int>(id) << "] Setting Operating Mode to " << op_mode);
        if (!retryWriteItem(id, param_name, static_cast<uint32_t>(op_mode))) {
          return false;
        }
      }
    }
    ...
    for (const auto & param : gpio.parameters) {
      ...
      if (param_name.find("Limit") != std::string::npos) {
        RCLCPP_INFO_STREAM(logger_, "[ID:" << static_cast<int>(id) << "] Seteando LIMIT: " << param_name << " = " << param.second);
  ...
  std::function<double(double)> conversion)
{
  for (size_t i = 0; i < outer_size; ++i) {
    for (size_t k = 0; k < outer_handlers.at(i).interface_name_vec.size(); ++k) {
      ...
      if (outer_iface == "Goal Current") {
        RCLCPP_INFO(logger_, "MAP: joint=%s, interface=%s, mapped_value=%f", 
          outer_handlers.at(i).name.c_str(), outer_iface.c_str(), value);
      }
